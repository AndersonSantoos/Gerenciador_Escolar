...
export const criarCargoController = async (req: Request, res: Response) => {
    ...
    try {
        const novoCargo = await criarCargo(funcao, isLeader, status);
...
export const getCargoControllerById = async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        const cargo = await getCargoById(parseInt(id, 10));
        if (!cargo) {
...
export const updateCargoControllerById = async (req: Request, res: Response) => {
    ...
    try {
        const cargoAtualizado = await updateCargoById(parseInt(id, 10), newData);
...
export const deleteCargoControllerById = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const result = await deleteCargoById(parseInt(id, 10));
        return result;
    } catch (error) {
